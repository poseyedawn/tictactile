...
const GameStatus: React.FC<GameStatusProps> = ({
  ...
  const renderStatusMessage = () => {
    if (status === 'won') {
      return <div className="flex items-center justify-center space-x-2 animate-fade-in">
      ...
      return <div className="flex items-center justify-center space-x-2">
          ...
          {currentPlayer === 'X' ? <X className="w-5 h-5 text-[hsl(var(--player-color))]" /> : <>
              <Circle className={cn("w-5 h-5 text-[hsl(var(--ai-color))]", aiThinking ? "thinking" : "")} />
              {aiThinking && <span className="text-sm italic">Thinking...</span>}
            </>}
        </div>;
    }
  };
  return <div className="w-full max-w-md mx-auto py-4 space-y-4">
      ...
      <div className="grid grid-cols-3 gap-4 text-center text-sm">
        <div className="p-2 rounded-md bg-[hsl(var(--control-bg))] theme-transition bg-orange-400">
